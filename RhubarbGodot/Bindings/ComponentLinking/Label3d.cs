...
namespace RhubarbVR.Bindings.ComponentLinking
{
	public sealed class LinkedLabel3D : GeometryInstance3DBase<TextLabel3D, Label3D>
	{
		...
		private void Font_LoadChange(RFont obj) {
			node.Font = LinkedComp.Font.Asset?.Inst is GodotFont font ? (font?.FontFile) : null;
		}
		...
		private void Dir_Changed(IChangeable obj) {
			node.TextDirection = LinkedComp.TextDir.Value switch {
				RTextDirection.Ltr => Direction.Ltr,
				RTextDirection.Rtl => Direction.Rtl,
				_ => TextServer.Direction.Auto,
			};
		}

		private void Width_Changed(IChangeable obj) {
			node.Width = LinkedComp.Width.Value;
		}

		private void TextAutoWrap_Changed(IChangeable obj) {
			node.AutowrapMode = LinkedComp.TextAutoWrap.Value switch {
		...
		private void LineSpacing_Changed(IChangeable obj) {
			node.LineSpacing = LinkedComp.LineSpacing.Value;
		}
		...
		private void VerticalAlignment_Changed(IChangeable obj) {
			node.VerticalAlignment = LinkedComp.VerticalAlignment.Value switch {
				RTextVerticalAlignment.Top => VerticalAlignment.Top,
				RTextVerticalAlignment.Bottom => VerticalAlignment.Bottom,
		...
		private void OutLineSize_Changed(IChangeable obj) {
			node.OutlineSize = LinkedComp.OutLineSize.Value;
		}

		private void FontSize_Changed(IChangeable obj) {
			node.FontSize = LinkedComp.FontSize.Value;
		}
		...
		private void OutlineModulate_Changed(IChangeable obj) {
			node.OutlineModulate = new Color(LinkedComp.OutlineModulate.Value.r, LinkedComp.OutlineModulate.Value.g, LinkedComp.OutlineModulate.Value.b, LinkedComp.OutlineModulate.Value.a);
		}
		...
		private void OutlineRenderPriority_Changed(IChangeable obj) {
			node.OutlineRenderPriority = LinkedComp.OutlineRenderPriority.Value;
		}
		...
		private void TextureFilter_Changed(IChangeable obj) {
			node.TextureFilter = LinkedComp.TextureFilter.Value switch {
				RElementTextureFilter.Linear => BaseMaterial3D.TextureFilterEnum.Linear,
				RElementTextureFilter.LinearMipmap => BaseMaterial3D.TextureFilterEnum.LinearWithMipmaps,
				RElementTextureFilter.NearestMipmap => BaseMaterial3D.TextureFilterEnum.NearestWithMipmaps,
				RElementTextureFilter.NearestMipmapAnisotropic => BaseMaterial3D.TextureFilterEnum.NearestWithMipmapsAnisotropic,
				RElementTextureFilter.LinearMipmapAnisotropic => BaseMaterial3D.TextureFilterEnum.LinearWithMipmapsAnisotropic,
				_ => BaseMaterial3D.TextureFilterEnum.Nearest,
			};
		}

		private void AlphaScissorThreshold_Changed(IChangeable obj) {
			node.AlphaScissorThreshold = LinkedComp.AlphaScissorThreshold;
		}

		private void AlphaCutout_Changed(IChangeable obj) {
			node.AlphaCut = LinkedComp.AlphaCutout.Value switch {
				RTextAlphaCutout.Discard => Label3D.AlphaCutMode.Discard,
				RTextAlphaCutout.Opaque_PrePass => Label3D.AlphaCutMode.OpaquePrepass,
				_ => Label3D.AlphaCutMode.Disabled,
			};
		}

		private void FixSize_Changed(IChangeable obj) {
			node.FixedSize = LinkedComp.FixSize.Value;
		}
		...
		private void TextOffset_Changed(IChangeable obj) {
			node.Offset = new Vector2(LinkedComp.TextOffset.Value.x, LinkedComp.TextOffset.Value.y);
		}
