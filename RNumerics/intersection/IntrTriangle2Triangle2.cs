	...
	public sealed class IntrTriangle2Triangle2
	{
		...
		public bool Test() {
			...
			for (i0 = 0, i1 = 2; i0 < 3; i1 = i0++) {
				...
				dir.x = _triangle0[i0].y - _triangle0[i1].y;
				dir.y = _triangle0[i1].x - _triangle0[i0].x;
				if (WhichSide(_triangle1, _triangle0[i1], dir) > 0) {
			...
			for (i0 = 0, i1 = 2; i0 < 3; i1 = i0++) {
				...
				dir.x = _triangle1[i0].y - _triangle1[i1].y;
				dir.y = _triangle1[i1].x - _triangle1[i0].x;
				if (WhichSide(_triangle0, _triangle1[i1], dir) > 0) {
		...
		public bool Find() {
			...
			for (var i = 0; i < 3; ++i) {
				Points[i] = _triangle1[i];
			}

			for (int i1 = 2, i0 = 0; i0 < 3; i1 = i0++) {
				// Clip against edge <V0[i1],V0[i0]>.
				var N = new Vector2d(
					_triangle0[i1].y - _triangle0[i0].y,
					_triangle0[i0].x - _triangle0[i1].x);
				var c = N.Dot(_triangle0[i1]);
				...
				if (Quantity == 0) {
					...
					Type = Quantity == 1 ? IntersectionType.Point : Quantity == 2 ? IntersectionType.Segment : IntersectionType.Polygon;
				}
			}
