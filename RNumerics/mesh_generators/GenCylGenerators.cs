...
namespace RNumerics
{
	...
	public sealed class TubeGenerator : MeshGenerator
	{
		...
		override public MeshGenerator Generate() {
			...
			var nRings = (ClosedLoop && NoSharedVertices) ? NV + 1 : NV;
			...
			var quad_strips = ClosedLoop ? NV : NV - 1;
			var nSpanTris = quad_strips * 2 * Slices;
			var nCapTris = (Capped && ClosedLoop == false) ? 2 * Slices : 0;
			triangles = new IndexArray3i(nSpanTris + nCapTris);

			var fCur = new Frame3f(Frame);
			var dv = CurveUtils.GetTangent(Vertices, 0, ClosedLoop);
			fCur.Origin = (Vector3f)Vertices[0];
			fCur.AlignAxis(2, (Vector3f)dv);
			var fStart = new Frame3f(fCur);

			var circumference = Polygon.ArcLength;
			var pathLength = CurveUtils.ArcLength(Vertices, ClosedLoop);
			...
			for (var ri = 0; ri < nRings; ++ri) {
				...
				// propagate frame
				var tangent = CurveUtils.GetTangent(Vertices, vi, ClosedLoop);
				...
				for (var j = 0; j < nRingSize; ++j) {
					var k = nStartR + j;
					var pv = Polygon.Vertices[j % Slices];
					var pvNext = Polygon.Vertices[(j + 1) % Slices];
				...
				var viNext = (ri + 1) % NV;
				var d = Vertices[vi].Distance(Vertices[viNext]);
			...
			// generate triangles
			var ti = 0;
			var nStop = (ClosedLoop && NoSharedVertices == false) ? nRings : (nRings - 1);
			for (var ri = 0; ri < nStop; ++ri) {
				var r0 = ri * nRingSize;
				var r1 = r0 + nRingSize;
			...
			if (Capped && ClosedLoop == false) {
				var c = OverrideCapCenter ? CapCenter : Polygon.Bounds.Center;
				...
				if (NoSharedVertices) {
					...
					for (var k = 0; k < Slices; ++k) {
						...
						var vuv = ((Polygon[k] - c).Normalized + Vector2d.One) * 0.5;
