...
namespace RNumerics
{
	...
	public class BSplineBasis
	{
		...
		public BSplineBasis(int numCtrlPoints, int degree, bool open) {
			...
			var factor = 1.0 / (mNumCtrlPoints - mDegree);
			if (mOpen) {
				for (i = 0; i <= mDegree; ++i) {
					mKnot[i] = 0;
				}
		...
		public BSplineBasis(int numCtrlPoints, int degree, double[] knots, bool bIsInteriorKnots) {
			...
			if (bIsInteriorKnots) {
				...
				for (var j = 0; i < mNumCtrlPoints; ++i, ++j) {
					mKnot[i] = knots[j];
				}
		...
		public void SetInteriorKnot(int j, double value) {
			if (!mUniform) {
				...
				var i = j + mDegree + 1;
				mKnot[i] = mDegree + 1 <= i && i <= mNumCtrlPoints ? value : throw new Exception("BSplineBasis.SetKnot: index out of range: " + j);
			}
			else {
		...
		protected int Initialize(int numCtrlPoints, int degree, bool open) {
			...
			var numKnots = mNumCtrlPoints + mDegree + 1;
		...
		protected double[,] Allocate() {
			var numRows = mDegree + 1;
			var numCols = mNumCtrlPoints + mDegree;
