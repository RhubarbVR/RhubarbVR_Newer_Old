...
namespace RNumerics.IK
{
	public partial class FullBodyIK
	{
		public class FingerIK
		{
			...
			FastAngle _notThumbYawThetaLimit = new FastAngle( 10.0f * MathUtil.DEG_2_RADF );
			FastAngle _notThumbPitchUThetaLimit = new FastAngle( 60.0f * MathUtil.DEG_2_RADF );
			FastAngle _notThumbPitchLThetaLimit = new FastAngle( 160.0f * MathUtil.DEG_2_RADF );

			FastAngle _notThumb0FingerIKLimit = new FastAngle( 60.0f * MathUtil.DEG_2_RADF );

			FastAngle _notThumb1PitchUTrace = new FastAngle( 5.0f * MathUtil.DEG_2_RADF );
			FastAngle _notThumb1PitchUSmooth = new FastAngle( 5.0f * MathUtil.DEG_2_RADF );
			FastAngle _notThumb1PitchUTraceSmooth = new FastAngle( 10.0f * MathUtil.DEG_2_RADF ); // _notThumb1PitchUTrace + _notThumb1PitchUSmooth
			FastAngle _notThumb1PitchLTrace = new FastAngle( 10.0f * MathUtil.DEG_2_RADF );
			FastAngle _notThumb1PitchLLimit = new FastAngle( 80.0f * MathUtil.DEG_2_RADF );
			...
			void _PrepareBranch2( int fingerType )
			{
				...
				if( fingerBranch.fingerLinks != null && fingerBranch.fingerLinks.Length > 0 && fingerBranch.fingerLinks[0].bone != null ) {
					Vector3f dirX = fingerEffector.defaultPosition - fingerBranch.fingerLinks[0].bone._defaultPosition;
				...
				if( fingerType == (int)FingerType.Thumb ) {
					...
					if( middleFingerBranch.fingerLinks.Length >= 1 ) {
						...
						Vector3f middleDirX = middleFingerLink0.bone._defaultPosition - _parentBone._defaultPosition;
						...
						if( fingerLinkLength >= 2 && fingerEffector._isSimulateFingerTips == false ) {
							...
							Vector3f thumbPosition0 = thumbFingerLink0.bone._defaultPosition;
							Vector3f thumbPosition1 = thumbFingerLink1.bone._defaultPosition;
							Vector3f thumbPosition2 = fingerEffector._defaultPosition;
							Vector3f thumb0to1 = thumbPosition1 - thumbPosition0;
							Vector3f thumb1to2 = thumbPosition2 - thumbPosition1;
							...
							Vector3f tempY = Vector3f.Cross( thumb0to1, thumb1to2 );
						...
						if( !isSolved && fingerLinkLength >= 3 ) {
							...
							Vector3f thumbPosition0 = thumbFingerLink0.bone._defaultPosition;
							Vector3f thumbPosition1 = thumbFingerLink1.bone._defaultPosition;
							Vector3f thumbPosition2 = thumbFingerLink2.bone._defaultPosition;
							Vector3f thumb0to1 = thumbPosition1 - thumbPosition0;
							Vector3f thumb1to2 = thumbPosition2 - thumbPosition1;
				...
				for( int n = 0; n != fingerBranch.fingerLinks.Length; ++n ) {
					...
					Vector3f sourcePosition = fingerLink.bone._defaultPosition;
					...
					if( n + 1 != fingerBranch.fingerLinks.Length ) {
						...
						if( !fingerBranch.effector._isSimulateFingerTips ) {
							...
							Vector3f tempTranslate = destPosition - sourcePosition;
                            sourceToDestLength = FastLength.FromVector3f( ref tempTranslate );
							if( sourceToDestLength.length > FLOAT_EPSILON ) {
								sourceToDestDirection = tempTranslate * (1.0f / sourceToDestLength.length);
                            } else {
								sourceToDestDirection = Vector3f.Zero;
                            }
                        }
					}
					...
					{
						Vector3f dirX = sourceToDestDirection;
					...
					if( fingerType == (int)FingerType.Thumb ) {
						...
						Vector3f dirX = fingerBranch.effector._defaultPosition - sourcePosition;
					...
					if( fingerBranch.fingerLinks.Length == 3 ) {
						...
						if( linkLength0 > linkLength2 ) {
							...
							if( norm_lengthD0 < min_cosTheta ) {
								...
								float beginLink_endSinTheta = MathUtil.Clamp01( (linkLength2 + lengthH0) * (1.0f / linkLength0) );
			...
			void _PrepareThumb()
			{
...

				{
					...
					Vector3f thumbToIndex = indexBeginLink.bone._defaultPosition - fingerLink0.bone._defaultPosition;
					Vector3f localThumbToIndex;
					...
					if( SAFBIKVecNormalize( ref localThumbToIndex ) ) {
						...
						_thumbBranch.thumb0_outerLimit = (float)System.Math.Sin( MathF.Max( -(SAFBIKAsin( _thumbBranch.thumb0_innerLimit ) - 40.0f * MathUtil.DEG_2_RADF), 0.0f ) );
						_thumbBranch.thumb0_upperLimit = MathF.Max( localThumbToIndex.y, 0.0f ); // upperLimit = height index 0
						_thumbBranch.thumb0_lowerLimit = (float)System.Math.Sin( MathF.Max( -(SAFBIKAsin( _thumbBranch.thumb0_upperLimit ) - 45.0f * MathUtil.DEG_2_RADF), 0.0f ) );
					}
				}
			...
			static float _ComputeJointBaseAngle(
				...
				bool isRight )
			{
				Vector3f linkToEnd = endPosition - beginPosition;
				Vector3f linkToNext = nextPosition - beginPosition;
				if( SAFBIKVecNormalize2( ref linkToEnd, ref linkToNext ) ) {
					Matrix3x3 linkToEndBasis;
					Vector3f dirX = isRight ? linkToEnd : -linkToEnd;
					SAFBIKComputeBasisFromXZLockX( out linkToEndBasis, dirX, rootBaseBasis.column2 );
					dirX = isRight ? linkToNext : -linkToNext;
					Vector3f dirY = linkToEndBasis.column2;
					Matrix3x3 linkToNextBasis;
					SAFBIKComputeBasisFromXZLockZ( out linkToNextBasis, ref dirX, ref dirY );

					float dotX = Vector3f.Dot( linkToEndBasis.column0, linkToNextBasis.column0 );
					float dotY = Vector3f.Dot( linkToEndBasis.column1, linkToNextBasis.column0 );
			...
			{
				Vector3f dirX = isRight ? effectorDirection : -effectorDirection;
				Vector3f dirZ;
				...
				ref _FingerIKParams fingerIKParams )
			{
				...
				Vector3f beginToEndDirection = endPosition - beginPosition;
				...
				Vector3f centerToBendingDirection = Vector3f.Cross( beginToEndDirection, bendingDirection );
				...
				float solveCosTheta = MathUtil.Lerp( fingerIKParams.beginLink_endCosTheta, 1.0f, MathUtil.Clamp01( (beginToEndLength - fingerIKParams.lengthD0) * fingerIKParams.lengthABCDInv ) );
				float solveSinTheta = SAFBIKSqrtClamp01( 1.0f - solveCosTheta * solveCosTheta );

				Vector3f solvedDirection = beginToEndDirection * solveCosTheta + centerToBendingDirection * solveSinTheta;
				...
				ref Vector3f bendingDirection )
			{
				...
				Vector3f beginToEndDirection = (endPosition - beginPosition) * (1.0f / beginToEndLength);
				if( beginToEndLength >= beginToEndBaseLength - IKEpsilon ) {
					return beginToEndDirection;
				}
				...
				Vector3f beginToInterDirection = beginToEndDirection * beginToInterTheta * beginToInterBaseLength
			...
			public bool Solve()
			{
				...
				Quaternionf parentBoneWorldRotation = _parentBone.worldRotation;
			...
			static Vector3f _GetEffectorPosition(
				...
				ref Matrix3x4 parentTransform )
			{
				if( rootBone != null && beginLinkBone != null && effector != null ) {
					...
					if( effector.positionWeight < 1.0f - IKEpsilon ) {
						Vector3f endLinkPosition;
						if( internalValues.continuousSolverEnabled ) {
						...
						Vector3f beginLinkPosition = parentTransform * (beginLinkBone._defaultPosition - rootBone._defaultPosition);

						Vector3f moveFrom = endLinkPosition - beginLinkPosition;
						Vector3f moveTo = effectorPosition - beginLinkPosition;
						...
						if( lengthFrom > IKEpsilon && lengthTo > IKEpsilon ) {
							Vector3f dirFrom = moveFrom * (1.0f / lengthFrom);
							Vector3f dirTo = moveTo * (1.0f / lengthTo);
							Vector3f dir = _LerpDir( ref dirFrom, ref dirTo, effector.positionWeight );
							float len = MathUtil.Lerp( lengthFrom, lengthTo, MathUtil.Clamp01( 1.0f - (1.0f - effector.positionWeight) * _positionLerpRate ) );
			...
			bool _SolveNotThumb( int fingerType, ref Matrix3x4 parentTransform )
			{
				...
				Vector3f beginLinkPosition = parentTransform * (beginLink.bone._defaultPosition - _parentBone._defaultPosition);
				Vector3f effectorPosition = _GetEffectorPosition( _internalValues, _parentBone, beginLink.bone, endEffector, fingerBranch.link0ToEffectorLength, ref parentTransform );
				Vector3f effectorTranslate = effectorPosition - beginLinkPosition;
				...
				Vector3f effectorDirection = effectorTranslate * (1.0f / effectorLength);
...

				{
					...
					Vector3f localEffectorDirection;
				...
				Vector3f solveDirY = Vector3f.Zero;
				Vector3f solveDirZ = Vector3f.Zero;
				if( !_SolveInDirect(
					isRight,
					ref solveDirY,
					ref solveDirZ,
				...
				if( isWarp ) {
					...
					Vector3f bendingLink0Position = parentTransform * (bendingLink0.bone._defaultPosition - _parentBone._defaultPosition);
					Vector3f bendingLink1Position = parentTransform * (bendingLink1.bone._defaultPosition - _parentBone._defaultPosition);
					Vector3f endPosition = parentTransform * (endEffector._defaultPosition - _parentBone._defaultPosition);

					Vector3f beginLinkDirX = Vector3f.Zero;

					{
						Vector3f beginLinkToBendingLink0Direction = bendingLink0Position - beginLinkPosition;
						Vector3f beginLinkToEndDirection = endPosition - beginLinkPosition;
						if( SAFBIKVecNormalize2( ref beginLinkToBendingLink0Direction, ref beginLinkToEndDirection ) ) {
							...
							if( SAFBIKComputeBasisFromXZLockX( out effBasis, isRight ? effectorDirection : -effectorDirection, solveDirZ ) ) {
								...
								if( SAFBIKComputeBasisFromXZLockZ( out bendBasis, isRight ? beginLinkToBendingLink0Direction : -beginLinkToBendingLink0Direction, effBasis.column2 ) &&
									...
									Vector3f effX = isRight ? effBasis.column0 : -effBasis.column0;
									Vector3f effY = effBasis.column1;
									Vector3f effZ = effBasis.column2;
									Vector3f bendX = isRight ? bendBasis.column0 : -bendBasis.column0;
									Vector3f bendY = bendBasis.column1;
									Vector3f endX = isRight ? endBasis.column0 : -endBasis.column0;
									Vector3f endY = endBasis.column1;

									// rotBendX ... begin to current bendLink0 basis.			
									float endBendDotX = Vector3f.Dot( bendX, endX ); // Cosine
									float endBendDotY = Vector3f.Dot( bendX, endY ); // Sine
									Vector3f rotBendX = _Rotate( ref effX, ref effY, endBendDotX, endBendDotY );

									imm_isUpper = (Vector3f.Dot( endY, effX ) >= 0.0f);

									bool imm_isLimitL = false;

									float endEffDotX = Vector3f.Dot( endX, effX );
									if( imm_isUpper ) {
										if( isWarp ) {
											...
											if( traceLimitUAngle <= IKEpsilon || endEffDotX < cosTraceLimitUAngle ) {
												Vector3f rotBendY = Vector3f.Cross( effZ, rotBendX );
					...
					if( !solveFingerIK ) {
						...
						Vector3f basedEffectorPosition = beginLinkPosition + effectorDirection * baseLength;

						Vector3f bendingLink0ToEffectorDirection = basedEffectorPosition - bendingLink0Position;
						Vector3f bendingLink0ToBendingLink0Direction = bendingLink1Position - bendingLink0Position;
						Vector3f bendingLink0ToEndDirection = endPosition - bendingLink0Position;
						...
						Vector3f bendingLink0DirX = Vector3f.Zero;
						...
						{
							Vector3f dirX = basedEffectorPosition - bendingLink1Position;
							if( !SAFBIKVecNormalize( ref dirX ) ) {
								return false;
							}
				...
				if( solveFingerIK ) {
					{
						Vector3f linkSolved = SolveFingerIK(
					...
					{
						Vector3f bendingLink0Position = beginLink.boneTransform * (bendingLink0.bone._defaultPosition - beginLink.bone._defaultPosition);

						// Forcefix:
						Vector3f bendingLink0ToEffector = effectorPosition - bendingLink0Position;
						...
						Vector3f linkSolved = SolveLimbIK(
					...
					{
						Vector3f bendingLink1Position = bendingLink0.boneTransform * (bendingLink1.bone._defaultPosition - bendingLink0.bone._defaultPosition);

						Vector3f dirX = effectorPosition - bendingLink1Position;
						if( !SAFBIKVecNormalize( ref dirX ) ) {
							return false;
						}
			...
			bool _SolveThumb( ref Matrix3x4 parentTransform )
			{
				...
				{
					Vector3f fingerLinkPosition0 = parentTransform * (fingerLink0.bone._defaultPosition - _parentBone._defaultPosition);
					var endEffector = fingerBranch.effector;
                    Vector3f effectorPosition = _GetEffectorPosition( _internalValues, _parentBone, fingerLink0.bone, endEffector, fingerBranch.link0ToEffectorLength, ref parentTransform );
					Vector3f effectorTranslate = effectorPosition - fingerLinkPosition0;
					float effectorLength = effectorTranslate.Magnitude;
...

					{
						...
						Vector3f dirX = effectorDirection;
					...
					{
						Vector3f fingerLinkPosition1 = fingerLink0.boneTransform * (fingerLink1.bone._defaultPosition - fingerLink0.bone._defaultPosition);
						Vector3f effectorTranslate1to3 = effectorPosition - fingerLinkPosition1;
						float effectorLength1to3 = effectorTranslate1to3.Magnitude;

						if( effectorLength1to3 < _thumbBranch.linkLength1to3 - IKEpsilon ) {
							Vector3f effectorTranslate0to3 = effectorPosition - fingerLink0.boneTransform.origin;
							...
							if( effectorLength0to3 > IKEpsilon ) {
								Vector3f baseDirection0to1 = fingerLinkPosition1 - fingerLink0.boneTransform.origin;
								if( SAFBIKVecNormalize( ref baseDirection0to1 ) ) {
									Vector3f effectorDirection0to3 = effectorTranslate0to3 * (1.0f / effectorLength0to3);
							...
							if( moveTheta < baseTheta ) {
								...
								if( newAngle > 0.01f * MathUtil.DEG_2_RADF ) {
									...
									if( moveLenAtoAD > IKEpsilon ) {
										...
										Vector3f newX = fingerLinkPosition1 - fingerLink0.boneTransform.origin;
...

					{
						// thumb1
						{
							...
							Vector3f dirX = effectorPosition - fingerLinkPosition1;
							if( !SAFBIKVecNormalize( ref dirX ) ) {
								return false;
							}
						...
						Vector3f effectorTranslate1to3 = effectorPosition - fingerLink1.boneTransform.origin;
						...
						if( moveThetaAtoB < _thumbBranch.thumb1_baseThetaAtoB ) {
							...
							if( newAngle > 0.01f * MathUtil.DEG_2_RADF ) {
								...
								float moveLenAtoAD = SAFBIKSqrt( moveLenASq2 - moveLenASq2 * SAFBIKCos( newAngle ) );
								{
									...
									Vector3f fingerLinkPosition2 = fingerLink1.boneTransform * (fingerLink2.bone._defaultPosition - fingerLink1.bone._defaultPosition);
									fingerLinkPosition2 += solveDirection * moveLenAtoAD;

									Vector3f newX = fingerLinkPosition2 - fingerLink1.boneTransform.origin;
									if( SAFBIKVecNormalize( ref newX ) ) {
										Vector3f dirY;
...

					{
						...
						Vector3f fingerLinkPosition2 = fingerLink1.boneTransform * (fingerLink2.bone._defaultPosition - fingerLink1.bone._defaultPosition);
						Vector3f dirX = effectorPosition - fingerLinkPosition2;
						if( !SAFBIKVecNormalize( ref dirX ) ) {
							return false;
						}
