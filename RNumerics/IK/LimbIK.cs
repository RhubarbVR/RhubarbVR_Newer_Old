...
namespace RNumerics.IK
{
	public partial class FullBodyIK
	{
		public class LimbIK
		{
			...
			Quaternionf _endEffectorToWorldRotation = Quaternionf.Identity;
			...
			Quaternionf _arm_bendingWorldToBeginBoneRotation = Quaternionf.Identity;
			// for Hand roll.
			Quaternionf _arm_endWorldToBendingBoneRotation = Quaternionf.Identity;
			...
			void _SyncDisplacement() {
				...
				if (_settings.syncDisplacement == SyncDisplacement.Everyframe || !_isSyncDisplacementAtLeastOnce) {
					...
					Vector3f beginToEndDir = _endBone._defaultPosition - _beginBone._defaultPosition;
					if (SAFBIKVecNormalize(ref beginToEndDir)) {
						if (_limbIKType == LimbIKType.Arm) {
							if (_limbIKSide == Side.Left) {
								beginToEndDir = -beginToEndDir;
							}
							Vector3f dirY = _internalValues.defaultRootBasis.column1;
							Vector3f dirZ = _internalValues.defaultRootBasis.column2;
							...
							beginToEndDir = -beginToEndDir;
							Vector3f dirX = _internalValues.defaultRootBasis.column0;
							Vector3f dirZ = _internalValues.defaultRootBasis.column2;
			...
			void _SolveBaseBasis(out Matrix3x3 baseBasis, ref Matrix3x3 parentBaseBasis, ref Vector3f effectorDir) {
				if (_limbIKType == LimbIKType.Arm) {
					Vector3f dirX = (_limbIKSide == Side.Left) ? -effectorDir : effectorDir;
					Vector3f basisY = parentBaseBasis.column1;
					Vector3f basisZ = parentBaseBasis.column2;
					...
					Vector3f dirY = -effectorDir;
					Vector3f basisX = parentBaseBasis.column0;
					Vector3f basisZ = parentBaseBasis.column2;
			...
			public void PresolveBeinding() {
				...
				Vector3f beginPos = _beginBone.worldPosition;
				Vector3f bendingPos = _bendingEffector.bone.worldPosition;
				Vector3f effectorPos = _endEffector.bone.worldPosition;
				Vector3f effectorTrans = effectorPos - beginPos;
				Vector3f bendingTrans = bendingPos - beginPos;
				...
				Vector3f effectorDir = effectorTrans * (1.0f / effectorLen);
				Vector3f bendingDir = bendingTrans * (1.0f / bendingLen);

				Matrix3x3 parentBaseBasis;
				Quaternionf parentBoneWorldRotation = _beginBone.parentBone.worldRotation;
				...
				Matrix3x3 toBasis;
				if (_limbIKType == LimbIKType.Arm) {
					Vector3f dirX = (_limbIKSide == Side.Left) ? -bendingDir : bendingDir;
					Vector3f basisY = parentBaseBasis.column1;
					Vector3f basisZ = parentBaseBasis.column2;
			...
			bool _PrefixLegEffectorPos_UpperNear(ref Vector3f localEffectorTrans) {
			...
			static bool _PrefixLegEffectorPos_Circular_Far(ref Vector3f localEffectorTrans, float effectorLength) {
				return _PrefixLegEffectorPos_Circular(ref localEffectorTrans, effectorLength, true);
			}

			static bool _PrefixLegEffectorPos_Circular(ref Vector3f localEffectorTrans, float effectorLength, bool isFar) {
			...
			static bool _PrefixLegEffectorPos_Upper_Circular_Far(ref Vector3f localEffectorTrans,
			...
			// Lefthand based.
			static void _ComputeLocalDirXZ(ref Vector3f localDir, out Vector3f localDirXZ) {
			...
			// Lefthand based.
			static void _ComputeLocalDirYZ(ref Vector3f localDir, out Vector3f localDirYZ) {
			...
			public bool PresolveInternal(
				...
				Vector3f bendingPos = _bendingEffector._hidden_worldPosition;
				Vector3f effectorPos = _endEffector._hidden_worldPosition;

				if (_bendingEffector.positionEnabled && _bendingEffector.pull > IKEpsilon) {
					Vector3f beginToBending = bendingPos - beginPos;
				...
				if (_bendingEffector.positionEnabled && _bendingEffector.pull > IKEpsilon) {
					Vector3f bendingToEffector = effectorPos - bendingPos;
				...
				Vector3f localEffectorDir = new Vector3f(0.0f, 0.0f, 1.0f);
				if (_limbIKType == LimbIKType.Arm) {
				...
				if (!_bendingEffector.positionEnabled) {
					...
					Vector3f presolvedBendingPos = Vector3f.Zero;
					...
					if (presolvedEnabled && _isPresolvedBending) {
						...
						if (presolvedBendingRate > IKEpsilon) {
							...
							if (_presolvedLerpTheta.cos < 1.0f - IKEpsilon) { // Lerp
								float presolvedFeedbackTheta = Vector3f.Dot(effectorDir, _presolvedEffectorDir);
					...
					if (presolvedBendingRate < 1.0f - IKEpsilon) {
						...
						if (_limbIKType == LimbIKType.Arm) {
							Vector3f dirX = (_limbIKSide == Side.Left) ? -baseBasis.column0 : baseBasis.column0;
							{
								...
								Vector3f localDir = (_limbIKSide == Side.Left) ? localEffectorDir : new Vector3f(-localEffectorDir.x, localEffectorDir.y, localEffectorDir.z);

								if (localDir.y < 0.0f) {
									elbowAngle = MathUtil.Lerp(elbowAngle, elbowLowerAngle, -localDir.y);
								}
								else {
									elbowAngle = MathUtil.Lerp(elbowAngle, elbowUpperAngle, localDir.y);
								}
								...
								if (_settings.limbIK.armEffectorBackfixEnabled) {
									...
									Vector3f localXZ; // X is reversed in RightSide.
									Vector3f localYZ;
										...
										if (localXZ.x < armEffectorBackCoreEndCosTheta) {
											...
											if (t > IKEpsilon) {
												...
												if (localYZ.y <= armEffectorBackCoreLowerSinTheta) {
													...
													float angle0 = MathUtil.Lerp(elbowAngle, targetAngle, r); // Lower
													float angle1 = MathUtil.Lerp(elbowAngle, targetAngle - 360.0f, r); // Upper
								...
								Vector3f dirY = parentBaseBasis.column1;
								Vector3f dirZ = Vector3f.Cross(baseBasis.column0, dirY);
				...
				{
					Vector3f beginToBendingTrans = bendingPos - beginPos;
					Vector3f intersectBendingTrans = beginToBendingTrans - effectorDir * Vector3f.Dot(effectorDir, beginToBendingTrans);
					float intersectBendingLen = intersectBendingTrans.Magnitude;

					if (intersectBendingLen > IKEpsilon) {
						Vector3f intersectBendingDir = intersectBendingTrans * (1.0f / intersectBendingLen);
						...
						if (bc2 > IKEpsilon) {
							...
							Vector3f beginToInterTranslate = effectorDir * effectorCosTheta * _beginToBendingLength
															+ intersectBendingDir * effectorSinTheta * _beginToBendingLength;
							Vector3f interToEndTranslate = effectorPos - (beginPos + beginToInterTranslate);
				...
				if (isSolved && _limbIKType == LimbIKType.Arm && _settings.limbIK.armEffectorInnerfixEnabled) {
					...
					if (localX > limitTheta) {
						...
						Vector3f interPos = beginPos + bendingDir * _beginToBendingLength;
						Vector3f endDir = effectorPos - interPos;
				...
				if (!isSolved) { // Failsafe.
					Vector3f bendingDir = bendingPos - beginPos;
			...
			public bool Solve() {
				...
				Quaternionf bendingBonePrevRotation = Quaternionf.Identity;
				Quaternionf endBonePrevRotation = Quaternionf.Identity;
			...
			public bool _SolveInternal() {
				...
				float effectorLen;
				Matrix3x3 baseBasis;
				Vector3f solvedBeginToBendingDir;
				Vector3f solvedBendingToEndDir;
				...
				if (_limbIKType == LimbIKType.Arm) {
					...
					Vector3f basisX = baseBasis.column0;
					Vector3f basisZ = baseBasis.column2;
			...
			bool _SolveEndRotation(bool isSolved, ref Quaternionf bendingBonePrevRotation, ref Quaternionf endBonePrevRotation) {
				float endRotationWeight = _endEffector.rotationEnabled ? _endEffector.rotationWeight : 0.0f;
				if (endRotationWeight > IKEpsilon) {
					Quaternionf endEffectorWorldRotation = _endEffector.worldRotation;
					...
					if (endRotationWeight < 1.0f - IKEpsilon) {
						...
						if (isSolved) {
							Quaternionf bendingBoneWorldRotation = _bendingBone.worldRotation;
			...
			void _EndRotationLimit() {
				...
				// Rotation Limit.
				Quaternionf tempRotation, endRotation, bendingRotation, localRotation;
				...
				if (_limbIKType == LimbIKType.Arm) {
					...
					if (isLimited) {
						localRotation = Quaternionf.AxisAngleD(axis, angle);
			...
			bool _RollInternal() {
				...
				if (_armRollBones != null && _armRollBones.Length > 0) {
					...
					Vector3f dirX = beginBoneBasis.column0;
					Vector3f dirY = bendingBoneBasis.column1;
					Vector3f dirZ = bendingBoneBasis.column2;
				...
				if (_elbowRollBones != null && _elbowRollBones.Length > 0) {
					...
					Vector3f dirZ = endBoneBasis.column2;
					Vector3f dirX = bendingBoneBasis.column0;
					Vector3f dirY = Vector3f.Cross(dirZ, dirX);
