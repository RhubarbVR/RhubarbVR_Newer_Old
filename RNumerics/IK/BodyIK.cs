	...
	public partial class FullBodyIK
	{
		public class BodyIK
		{
				...
				if (_settings.syncDisplacement == SyncDisplacement.Everyframe || !_isSyncDisplacementAtLeastOnce) {
					...
					if (_shoulderBones != null) {
						for (int i = 0; i != 2; ++i) {
							...
							Vector3f dirY = _shoulderBones[i]._localAxisBasis.column1;
					...
					if (_nearArmBones != null) {
						...
						Vector3f dirX = _nearArmBones[1]._defaultPosition - _nearArmBones[0]._defaultPosition;
						Vector3f dirY = _defaultCenterArmPos - _defaultCenterLegPos;
					...
					if (_spineBones != null) {
						if ((_nearArmBones != null || _legBones != null) && _neckBone != null && _neckBone.transformIsAlive) {
							//Vector3f armDirX = (_nearArmBones != null)
							...
							Vector3f armDirX = _internalValues.defaultRootBasis.column0;
							...
							for (int i = 0; i < spineLength - 1; ++i) {
								...
								Vector3f prevPos = (i != 0) ? _spineBones[i - 1]._defaultPosition : _defaultCenterLegPos;
								Vector3f dirY0 = _defaultCenterArmPos - prevPos;
								Vector3f dirY1 = _defaultCenterArmPos - _spineBones[i]._defaultPosition;
			...
			public bool Solve() {
				...
				if (_settings.bodyIK.computeWorldIIKTransform) {
			...
			bool _UpperSolve() {
				...
				Vector3f baseCenterLegPos = Vector3f.Zero; // for continuousSolver
				...
				Vector3f targetCenterArmPos = temp.targetCenterArmPos;
				Vector3f targetCenterArmDir = temp.targetCenterArmDir;
				Vector3f currentCenterArmPos = temp.currentCenterArmPos;
				Vector3f currentCenterArmDir = temp.currentCenterArmDir;

				Vector3f centerArmDirX = _LerpDir(ref currentCenterArmDir, ref targetCenterArmDir, upperLerpDir1Rate);
				Vector3f centerArmDirX2 = _LerpDir(ref currentCenterArmDir, ref targetCenterArmDir, upperLerpDir2Rate);
				...
				if (_settings.bodyIK.spineLimitEnabled) {
					...
					if (_settings.bodyIK.spineAccurateLimitEnabled) { // Quaternionf lerp.
						float fromToX = Vector3f.Dot(centerArmDirX, centerArmDirX2);
						float fromToXAng = SAFBIKAcos(fromToX);
						if (fromToXAng > spineLimitAngleX) {
							Vector3f axisDir = Vector3f.Cross(centerArmDirX, centerArmDirX2);
							if (SAFBIKVecNormalize(ref axisDir)) {
								Quaternionf q = Quaternionf.AxisAngleD(axisDir, _settings.bodyIK.spineLimitAngleX);
						...
						float fromToY = Vector3f.Dot(centerArmDirY, centerArmDirY2);
						float fromToYAng = SAFBIKAcos(fromToY);
						if (fromToYAng > spineLimitAngleY) {
							Vector3f axisDir = Vector3f.Cross(centerArmDirY, centerArmDirY2);
							if (SAFBIKVecNormalize(ref axisDir)) {
								Quaternionf q = Quaternionf.AxisAngleD(axisDir, _settings.bodyIK.spineLimitAngleY);
								Matrix3x3 rotateBasis;
								SAFBIKMatSetRot(out rotateBasis, ref q);
						...
						if (fromToXAng > spineLimitAngleX) {
							if (fromToXAng > IKEpsilon) {
								...
								Vector3f dirX2Balanced = Vector3f.Lerp(centerArmDirX, centerArmDirX2, balancedRate);
						...
						if (fromToYAng > spineLimitAngleY) {
							if (fromToYAng > IKEpsilon) {
								...
								Vector3f dirY2Balanced = Vector3f.Lerp(centerArmDirY, centerArmDirY2, balancedRate);
				...
				Vector3f presolveCenterLegPos = temp.centerLegPos; // for continuousSolverEnabled

				// for eyes.
				// presolvedCenterLegPos2 = presolveCenterLegPos + presolved postTranslate.
				Vector3f presolvedCenterLegPos2 = temp.centerLegPos;
				if (eyesRate > IKEpsilon) {
					Vector3f source = temp.centerLegPos + centerArmDirY2 * _defaultCenterLegToCeterArmLen;
				...
				if (eyesRate > IKEpsilon) {
					...
					if (SAFBIKComputeBasisFromXYLockY(out toBasis, ref centerArmDirX2, ref centerArmDirY2)) {
						...
						Vector3f eyeDir = _eyesEffector.worldPosition - eyePos; // Memo: Not use _eyesEffector._hidden_worldPosition

						{
							...
							{
								Vector3f xDir = toBasis.column0;
								Vector3f yDir = toBasis.column1;
								Vector3f zDir = eyeDir;
					...
					if (SAFBIKComputeBasisFromXYLockY(out toBasis, ref centerArmDirX, ref centerArmDirY)) {
						...
						if (_internalValues.animatorEnabled) {
							...
							if (continuousSolverEnabled && stableCenterLegRate > IKEpsilon) {
								...
								Vector3f solveDirY = centerArmPos - presolveCenterLegPos;
								Vector3f solveDirX = centerArmDirX;
							...
							Matrix3x3 fromBasis;
							Vector3f currentDirX = temp.nearArmPos[1] - temp.nearArmPos[0];
							Vector3f currentDirY = (temp.nearArmPos[1] + temp.nearArmPos[0]) * 0.5f - temp.centerLegPos;
...

				{
					...
					Vector3f centerLegBasisX = temp.centerLegBasis.column0;
					Vector3f centerArmPosY2 = centerArmDirY2 * centerLegToArmLength + temp.centerLegPos;
					...
					for (int i = 0; i != spineLength; ++i) {
						...
						Vector3f origPos = temp.spinePos[i];

						if (i + 1 == spineLength) {
							Vector3f currentDirX = temp.nearArmPos[1] - temp.nearArmPos[0];
							Vector3f currentDirY = temp.centerArmPos - origPos;

							Vector3f targetDirX = centerArmDirX2;
							Vector3f targetDirY = centerArmPosY2 - origPos;
							...
							Vector3f childPos = (i + 1 == spineLength) ? temp.neckPos : temp.spinePos[i + 1];
							Vector3f prevPos = (i != 0) ? temp.spinePos[i - 1] : temp.centerLegPos;

							Vector3f currentDirX = temp.nearArmPos[1] - temp.nearArmPos[0];
							Vector3f currentDirY = childPos - origPos;

							Vector3f targetDirX = centerArmDirX2;
							Vector3f targetDirY = centerArmPosY2 - prevPos;
							Vector3f targetDirY2 = centerArmPosY2 - origPos;
			...
			void _UpperSolve_PresolveBaseCenterLegTransform(out Vector3f centerLegPos, out Matrix3x3 centerLegBasis) {
				...
				if (isLimited) {
					Vector3f vecX = centerLegBasis.column0 * _defaultCenterLegHalfLen;
			...
			void _UpperSolve_Transform(int origIndex, ref Matrix3x3 transformBasis) {
				...
				Vector3f origPos = (origIndex == -1) ? temp.centerLegPos : temp.spinePos[origIndex];
			...
			bool _UpperSolve_PreTranslate2(out Vector3f translate, ref CachedRate01 translateRate, ref CachedRate01 stableRate, ref Vector3f stableCenterLegPos) {
				...
				if (translateEnabled) {
					...
					if (continuousSolverEnabled && stableRate.isGreater0) {
						Vector3f extraTranslate = stableCenterLegPos - temp.centerLegPos;
					...
					if (_hipsEffector.positionEnabled && _hipsEffector.pull > IKEpsilon) {
						Vector3f extraTranslate = _hipsEffector._hidden_worldPosition - temp.centerLegPos;
			...
			void _UpperSolve_Translate2(ref CachedRate01 translateRate, ref CachedRate01 stableRate, ref Vector3f stableCenterLegPos) {
				Vector3f translate;
			...
			void _LowerSolve(bool firstPass) {
				...
				if (temp.PrepareLowerRotation(0)) {
					Vector3f centerLegBoneY = temp.centerLegBasis.column1;
					for (int i = 0; i < 2; ++i) {
						if (temp.legs.endPosEnabled[i] && temp.legs.targetBeginPosEnabled[i]) {
							Vector3f legDir = temp.legs.targetBeginPos[i] - temp.legs.beginPos[i];
							if (SAFBIKVecNormalize(ref legDir)) {
								float legDirFeedbackRate = Vector3f.Dot(centerLegBoneY, -legDir);
#if false
								legDirFeedbackRate = Mathf.Asin( Mathf.Clamp( legDirFeedbackRate, -1.0f, 1.0f ) );
								legDirFeedbackRate *= 1.0f / (90.0f * Mathf.Deg2Rad);
								legDirFeedbackRate = MathUtil.Clamp01( legDirFeedbackRate );
#else // Faster
								legDirFeedbackRate = MathUtil.Clamp01(legDirFeedbackRate);
#endif
			...
			void _ComputeWorldTransform() {
				...
				if (_hipsBone != null && _hipsBone.transformIsAlive && temp.spinePos != null && temp.spinePos.Length > 0 && _neckBone != null && _neckBone.transformIsAlive) {
					Vector3f hipsToSpineDirX = new Vector3f(1.0f, 0.0f, 0.0f);

					Vector3f dirX = temp.legs.beginPos[1] - temp.legs.beginPos[0];
					Vector3f dirY = temp.spinePos[0] - (temp.legs.beginPos[1] + temp.legs.beginPos[0]) * 0.5f;
					...
					if (SAFBIKVecNormalize(ref dirY) && SAFBIKComputeBasisFromXYLockY(out boneBasis, ref dirX, ref dirY)) {
						...
						if (_hipsBone.isWritebackWorldPosition) {
							...
							Vector3f inv_defaultLocalTranslate = -_spineBone._defaultLocalTranslate;
					...
					for (int i = 0; i != spineLength; ++i) {
						...
						if (i + 1 == spineLength) {
							...
							dirX = hipsToSpineDirX;
							if (temp.nearArmPos != null) {
								Vector3f dirX0 = temp.nearArmPos[1] - temp.nearArmPos[0];
			...
			bool _PrepareSolverInternal() {
				...
				if (_spineBones != null) {
					...
					float spineDirXRate = MathUtil.Clamp01(_settings.bodyIK.spineDirXLegToArmRate);
				...
				if (_solverInternal == null) {
					...
					if (_spineUBone != null) {
						if (_shoulderBones != null || _armBones != null) {
							...
							Vector3f dirY = nearArmBones[1]._defaultPosition + nearArmBones[0]._defaultPosition;
							Vector3f dirX = nearArmBones[1]._defaultPosition - nearArmBones[0]._defaultPosition;
							dirY = dirY * 0.5f - _spineUBone._defaultPosition;
							Vector3f dirZ = Vector3f.Cross(dirX, dirY);
			...
			void _PresolveHips() {
				...
				if (rotationEnabled) {
					Quaternionf centerLegRotationTo = _hipsEffector.worldRotation * Inverse(_hipsEffector._defaultRotation);
					Quaternionf centerLegRotationFrom;
					SAFBIKMatGetRot(out centerLegRotationFrom, ref centerLegBasis);

					Quaternionf centerLegRotation = centerLegRotationTo * Inverse(centerLegRotationFrom);
				...
				if (positionEnabled) {
					...
					Vector3f hipsEffectorWorldPosition = _hipsEffector._hidden_worldPosition;
					Vector3f centerLegPos;
					...
					Vector3f translate = centerLegPos - temp.centerLegPos;
			...
			void _ResetTransforms() {
				...
				Vector3f centerLegPos = Vector3f.Zero;
				_GetBaseCenterLegTransform(out centerLegPos, out centerLegBasis);
				_ResetCenterLegTransform(ref centerLegPos, ref centerLegBasis);
			}

			void _GetBaseCenterLegTransform(out Vector3f centerLegPos, out Matrix3x3 centerLegBasis) {
			...
			void _ResetCenterLegTransform(ref Vector3f centerLegPos, ref Matrix3x3 centerLegBasis) {
				var temp = _solverInternal;
				Assert(temp != null);

				Vector3f defaultCenterLegPos = _defaultCenterLegPos;
			...
			void _ResetShoulderTransform() {
				...
				Vector3f dirY = temp.neckPos - temp.spineUPos;
				Vector3f dirX = temp.nearArmPos[1] - temp.nearArmPos[0];
				...
				if (SAFBIKVecNormalize(ref dirY) && SAFBIKComputeBasisFromXYLockY(out boneBasis, ref dirX, ref dirY)) {
					...
					Vector3f tempPos = temp.spineUPos;
					for (int i = 0; i != 2; ++i) {
			...
			class SolverInternal
			{
				public class Limb
				{
					...
					public void _ClearEnvTargetBeginPos(int i, ref Vector3f beginPos) {
					...
					static bool _SolveTargetBeginPos(ref Vector3f targetBeginPos, ref Vector3f targetEndPos, ref FastLength targetBeginToEndLength, float endPull) {
						Vector3f beginToEnd = (targetEndPos - targetBeginPos);
				...
				public void _SetCenterArmPos(ref Vector3f centerArmPos) {
					_isDirtyCenterArmPos = false;
					_centerArmPos = centerArmPos;
				}

				public void _SetCenterLegPos(ref Vector3f centerLegPos) {
					_isDirtyCenterLegPos = false;
				...
				public void _UpdateCenterLegBasis() {
					if (_isDirtyCenterLegBasis) {
						...
						if (this.spinePos != null && this.spinePos.Length > 0 && legPos != null) {
							Vector3f dirX = legPos[1] - legPos[0];
							Vector3f dirY = this.spinePos[0] - this.centerLegPos;
							Vector3f dirZ = Vector3f.Cross(dirX, dirY);
							dirX = Vector3f.Cross(dirY, dirZ);
							if (SAFBIKVecNormalize3(ref dirX, ref dirY, ref dirZ)) {
				...
				public void _UpdateSpineUBasis() {
					if (_isDirtySpineUBasis) {
						...
						Vector3f dirY = (this.shoulderPos != null) ? (this.shoulderPos[1] + this.shoulderPos[0]) : (this.armPos[1] + this.armPos[0]);
						dirY = dirY * 0.5f - this.spineUPos;
						Vector3f dirX = (this.shoulderPos != null) ? (this.shoulderPos[1] - this.shoulderPos[0]) : (this.armPos[1] - this.armPos[0]);
						Vector3f dirZ = Vector3f.Cross(dirX, dirY);
						dirX = Vector3f.Cross(dirY, dirZ);
						if (SAFBIKVecNormalize3(ref dirX, ref dirY, ref dirZ)) {
				...
				struct _UpperSolverPreArmsTemp
				{
					...
					public static _UpperSolverPreArmsTemp Alloc() {
						...
						r.shoulderPos = new Vector3f[2];
						r.armPos = new Vector3f[2];
				...
				struct _UpperSolverArmsTemp
				{
					...
					public static _UpperSolverArmsTemp Alloc() {
						...
						r.nearArmPos = null; // shoulderPos / armPos
						r.shoulderEnabled = false;
						r.centerArmPos = Vector3f.Zero;
						r.centerArmDir = Vector3f.Zero;
						return r;
					}
				}
				...
				struct _UpperSolverTemp
				{
					...
					public static _UpperSolverTemp Alloc() {
						...
						r.targetArmPos = new Vector3f[2];
						r.targetNeckPos = new Vector3f();
						r.targetHeadPos = new Vector3f();
				...
				void _SolveArmsToArms(ref _UpperSolverArmsTemp armsTemp, float armPull, int idx0) {
					Vector3f targetArmPos = _upperSolverTemp.targetArmPos[idx0];
				...
				void _SolveArmsToNeck(ref _UpperSolverArmsTemp armsTemp, float neckToFullArmPull, int idx0) {
					Vector3f nearArmPos0 = armsTemp.nearArmPos[idx0];
				...
				public Vector3f currentCenterArmDir
				{
					get {
						if (this.shoulderPos != null) {
							Vector3f dir = (this.shoulderPos[1] - this.shoulderPos[0]);
							if (SAFBIKVecNormalize(ref dir)) {
								return dir;
							}
						}
						else if (this.armPos != null) {
							Vector3f dir = (this.armPos[1] - this.armPos[0]);
							if (SAFBIKVecNormalize(ref dir)) {
								return dir;
							}
						}
				...
				public bool UpperSolve() {
					...
					if (bodyMovingfixRate > IKEpsilon || headMovingfixRate > IKEpsilon) {
						Vector3f headMove = Vector3f.Zero;
						Vector3f bodyMove = Vector3f.Zero;
						...
						Vector3f totalMove = new Vector3f();
					...
					if (headMovingfixRate < 1.0f - IKEpsilon || bodyMovingfixRate < 1.0f - IKEpsilon) {
						if (headPull > IKEpsilon || neckPull > IKEpsilon) {
							if (headMovingfixRate < 1.0f - IKEpsilon && headPull > IKEpsilon) {
								Vector3f tempNeckPos = _upperSolverPreArmsTemp.neckPos;
							...
							for (int i = 0; i != 2; ++i) {
								if (bodyMovingfixRate < 1.0f - IKEpsilon && neckPull > IKEpsilon) {
									Vector3f tempNearArmPos = _upperSolverPreArmsTemp.nearArmPos[i];
				...
				Vector3f[] _tempArmPos = new Vector3f[2];
				Vector3f[] _tempArmToElbowDir = new Vector3f[2];
				Vector3f[] _tempElbowToWristDir = new Vector3f[2];
				...
				Vector3f[] _tempArmToElbowDefaultDir = new Vector3f[2];
					...
					for (int i = 0; i != 2; ++i) {
						if (_tempElbowPosEnabled[i]) {
							...
							if (theta >= -FLOAT_EPSILON && theta <= FLOAT_EPSILON) {
								...
								Vector3f tempShoulderPos = this.shoulderPos[i];
								Vector3f tempDir = _tempArmPos[i] - tempShoulderPos;
								SAFBIKVecNormalize(ref tempDir);
								Vector3f resultDir;
								SAFBIKMatMultVec(out resultDir, ref m, ref tempDir);
								Vector3f destArmPos = tempShoulderPos + resultDir * shoulderToArmLength[i];
				...
				public bool _PrepareLimbRotation(Limb limb, int i, int origIndex, ref Vector3f beginPos) {
					Assert(i < 2);
					this.origTheta[i] = 0.0f;
					this.origAxis[i] = new Vector3f(0.0f, 0.0f, 1.0f);
					...
					Vector3f origPos = (origIndex == -1) ? this.centerLegPos : this.spinePos[origIndex];
				...
				bool _SolveLimbRotation(Limb limb, int origIndex, out Quaternionf origRotation) {
					...
					Quaternionf origRotation0 = _GetRotation(ref this.origAxis[0], this.origTheta[0], this.origFeedbackRate[0] * 0.5f);
					Quaternionf origRotation1 = _GetRotation(ref this.origAxis[1], this.origTheta[1], this.origFeedbackRate[1] * 0.5f);
				...
				bool _PrepareLimbTranslate(Limb limb, int i, ref Vector3f beginPos) {
					this.origTranslate[i] = Vector3f.Zero;
					if (limb.targetBeginPosEnabled[i]) {
				...
				bool _SolveLimbTranslate(Limb limb, out Vector3f origTranslate) {
				...
				public void LowerTranslateBeginOnly(ref Vector3f origTranslate) {
					_LimbTranslateBeginOnly(this.legs, ref origTranslate);
					_centerLegPos += origTranslate; // Optimized.
				}

				void _LimbTranslateBeginOnly(Limb limb, ref Vector3f origTranslate) {
					for (int i = 0; i < 2; ++i) {
				...
				public void SolveShoulderToArmInternal(int i, ref Vector3f destArmPos) {
					...
					if (!IsFuzzy(ref armPos[i], ref destArmPos)) {
						Vector3f dirX = destArmPos - this.shoulderPos[i];
						if (SAFBIKVecNormalize(ref dirX)) {
			...
			static bool _ComputeCenterLegBasis(
				...
				Vector3f dirX = rightLegPos - leftLegPos;
				Vector3f dirY = spinePos - (rightLegPos + leftLegPos) * 0.5f;
			...
			static bool _KeepMaxLength(ref Vector3f posTo, ref Vector3f posFrom, float keepLength) {
				Vector3f v = posTo - posFrom;
			...
			static bool _KeepMaxLength(ref Vector3f posTo, ref Vector3f posFrom, ref FastLength keepLength) {
				Vector3f v = posTo - posFrom;
				float len = SAFBIKVecLength(ref v);
			...
			static bool _KeepLength(ref Vector3f posTo, ref Vector3f posFrom, float keepLength) {
				Vector3f v = posTo - posFrom;
				float len = SAFBIKVecLength(ref v);
				if (len > IKEpsilon) {
					v = v * (keepLength / len);
			...
			static Quaternionf _GetRotation(ref Vector3f axisDir, float theta, float rate) {
				if ((theta >= -IKEpsilon && theta <= IKEpsilon) || (rate >= -IKEpsilon && rate <= IKEpsilon)) {
					return Quaternionf.Identity;
				}
				else {
					return Quaternionf.AxisAngleD(axisDir, SAFBIKAcos(theta) * rate * MathUtil.RAD_2_DEGF);
				}
			}
			...
			static float _GetLerpRateFromPull2(float pull0, float pull1) {
				if (pull0 > FLOAT_EPSILON && pull1 > FLOAT_EPSILON) {
					return MathUtil.Clamp01(pull1 / (pull0 + pull1));
				}
