...
namespace OpusDotNet
{
	...
	public sealed class OpusEncoder : IDisposable
	{
		...
		/// <returns></returns>
		public int GetMaxDataBytes(int pcmLength, int bitRate) {
			var frames = pcmLength / Channels / 2;
			return (frames * (bitRate / 8)) + 10000;
		}
		...
		public int EncodeFloat(byte[] pcmBytes, int samples, byte[] opusBytes, int opusLength) {
			if (!NativeLib.Load()) {
				throw new Exception("Failed to load opus lib");
			}
			if (pcmBytes == null) {
