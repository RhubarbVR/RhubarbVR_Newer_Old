...
//		public Linker<Colorb> WindowTint;
...
//			pbutton.Size.Value = new Vector2f(((0.3f + (Engine.UISettings.gutter * 2)) / 2) - (Engine.UISettings.gutter / 2), 0);
//			pbutton.Text.Value = "<";
...
//			RLog.Info("Spawned Node " + scriptNode.Text);

//		}

//		public void LoadScriptNodes() {
//			RLog.Info($"Loading node list with Input {InputType.Value?.GetFormattedName() ?? "Null"} Output {OutPutType.Value?.GetFormattedName() ?? "Null"}");
...
//				button.Size.Value = new Vector2f(0.1f, 0.05f);
...
//			}
//			else {
//				RLog.Err($"Uknown node type: {node.GetType().GetFormattedName()}");
//			}
//		}

//		public override void OnGainFocus() {
//			if (WindowTint.Linked) {
//				WindowTint.LinkedValue = Colorf.White;
//			}
...
//			if (WindowTint.Linked) {
//				WindowTint.LinkedValue = new Colorf(1f, 0.5f, 0.5f);
//			}
//			if (WindowText.Linked) {
