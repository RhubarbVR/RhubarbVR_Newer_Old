		...
		private void ProcessController(bool isMain) {
			...
			switch (Engine.inputManager.GetHand(isMain)) {
				case Handed.Left:
					...
					break;
				case Handed.Right:
					ProcessGlobalRotToUserRootMovement(AddToMatrix, Matrix.R(Quat.FromAngles(0, 180, 90)) * LocalUser.userRoot.Target?.rightHand.Target?.GlobalTrans ?? UserRootEnity.GlobalTrans);
					break;
				default:
					break;
			}
