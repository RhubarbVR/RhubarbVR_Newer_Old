...
namespace RhuEngine.Components
{
	...
	public class LongEditor : Editor
	{
		public Linker<long> Linker;
		...
		public string EditStringText
		{
			get {
				try {
					return GetValue.Target is not null ? ((long)GetValue.Target.Invoke()).ToString() : Linker.Linked ? Linker.LinkedValue.ToString() : string.Empty;
				}
			...
			set {
				try {
					if (SetValue.Target is not null) {
						SetValue.Target.Invoke(long.Parse(value));
					}
					else {
						if (Linker.Linked) {
							Linker.LinkedValue = long.Parse(value);
						}
					}
				}
				catch { }
		...
		public long Number
		{
			get {
				try {
					return GetValue.Target is not null ? ((long)GetValue.Target.Invoke()) : Linker.Linked ? Linker.LinkedValue : 0;
				}
				catch { return 0; }
			}
			set {
