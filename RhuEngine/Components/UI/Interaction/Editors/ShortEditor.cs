...
namespace RhuEngine.Components
{
	...
	public class ShortEditor : Editor
	{
		public Linker<short> Linker;
			...
			set {
				try {
					if (SetValue.Target is not null) {
						SetValue.Target.Invoke(short.Parse(value));
					}
					else {
						if (Linker.Linked) {
							Linker.LinkedValue = short.Parse(value);
						}
					}
				}
				catch { }
		...
		public short Number
		{
			get {
				try {
					return GetValue.Target is not null ? ((short)GetValue.Target.Invoke()) : Linker.Linked ? Linker.LinkedValue : short.MaxValue;
				}
				catch { return 0; }
			}
			set {
