...
namespace RhuEngine.Components
{
	...
	public class ByteEditor : Editor
	{
		public Linker<byte> Linker;
		...
		public string EditStringText
		{
			get {
				try {
					return GetValue.Target is not null ? ((byte)GetValue.Target.Invoke()).ToString() : Linker.Linked ? Linker.LinkedValue.ToString() : string.Empty;
				}
			...
			set {
				try {
					if (SetValue.Target is not null) {
						SetValue.Target.Invoke(byte.Parse(value));
					}
					else {
						if (Linker.Linked) {
							Linker.LinkedValue = byte.Parse(value);
						}
					}
				}
				catch { }
		...
		public byte Number
		{
			get {
				try {
					return GetValue.Target is not null ? ((byte)GetValue.Target.Invoke()) : Linker.Linked ? Linker.LinkedValue : byte.MaxValue;
				}
				catch { return 0; }
			}
			set {
