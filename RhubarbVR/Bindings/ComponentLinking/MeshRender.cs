...
namespace RhubarbVR.Bindings.ComponentLinking
{
	...
	public abstract class GeometryInstance3DBase<T, T2> : VisualInstance3DBase<T, T2> where T : RhuEngine.Components.GeometryInstance3D, new() where T2 : GDExtension.GeometryInstance3D, new()
	{
		...
		private void FadeMode_Changed(IChangeable obj) {
			RenderThread.ExecuteOnEndOfFrame(() => node.VisibilityRangeFadeModeValue = LinkedComp.FadeMode.Value switch { RFadeMode.Self => VisibilityRangeFadeMode.Self, RFadeMode.Dependencies => VisibilityRangeFadeMode.Dependencies, _ => VisibilityRangeFadeMode.Disabled, });
		}
		...
		private void LightMapScale_Changed(IChangeable obj) {
			RenderThread.ExecuteOnEndOfFrame(() => node.GiLightmapScale = LinkedComp.LightMapScale.Value switch { RLightMapScale.TwoX => LightmapScale._2x, RLightMapScale.FourX => LightmapScale._4x, RLightMapScale.EightX => LightmapScale._8x, _ => LightmapScale._1x, });
		}
		...
		private void CastShadows_Changed(IChangeable obj) {
			RenderThread.ExecuteOnEndOfFrame(() => node.CastShadow = LinkedComp.CastShadows.Value switch {
				ShadowCast.On => GDExtension.GeometryInstance3D.ShadowCastingSetting.On,
				ShadowCast.TwoSided => GDExtension.GeometryInstance3D.ShadowCastingSetting.DoubleSided,
				ShadowCast.ShadowsOnly => GDExtension.GeometryInstance3D.ShadowCastingSetting.ShadowsOnly,
				_ => GDExtension.GeometryInstance3D.ShadowCastingSetting.Off,
			});
		}
	...
	public abstract class VisualInstance3DBase<T, T2> : WorldPositionLinked<T, T2> where T : RhuEngine.Components.VisualInstance3D, new() where T2 : GDExtension.VisualInstance3D, new()
	{
		public override void Init() {
			base.Init();
