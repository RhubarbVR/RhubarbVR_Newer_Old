	...
	public abstract class ButtonBase<T, T2> : UIElementLinkBase<T, T2> where T : ButtonBase, new() where T2 : BaseButton, new()
	{
		...
		private void Node_GuiInput(InputEvent @event) {
			if (@event is InputEventMouse mouse) {
				...
				switch (GetSideFromMouseID((int)mouse.Device)) {
					case Handed.Left:
						LeftPos = newPos;
						break;
					case Handed.Right:
				...
				LinkedComp.LastHanded = GetSideFromMouseID((int)mouse.Device);
			}
		...
		private void ActionMode_Changed(IChangeable obj) {
			RenderThread.ExecuteOnEndOfFrame(() =>
				node.ActionModeValue = LinkedComp.ActionMode.Value switch {
					RButtonActionMode.Relases => BaseButton.ActionMode.Release,
					_ => BaseButton.ActionMode.Press,
				});
		}
