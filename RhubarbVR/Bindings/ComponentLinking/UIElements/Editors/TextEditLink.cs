...
{
	public abstract class TextEditBase<T, T2> : UIElementLinkBase<T, T2> where T : RhuEngine.Components.TextEdit, new() where T2 : GDExtension.TextEdit, new()
	{
		...
		private void LineCaret_Changed(IChangeable obj) {
			RenderThread.ExecuteOnEndOfFrame(() => node.CaretTypeValue = LinkedComp.LineCaret.Value ? GDExtension.TextEdit.CaretType.Line : GDExtension.TextEdit.CaretType.Block);
		}
		...
		private void WrapText_Changed(IChangeable obj) {
			RenderThread.ExecuteOnEndOfFrame(() => node.WrapMode = LinkedComp.WrapText ? GDExtension.TextEdit.LineWrappingMode.None : GDExtension.TextEdit.LineWrappingMode.Boundary);
		}
	...
	public sealed class TextEditLink : TextEditBase<RhuEngine.Components.TextEdit, GDExtension.TextEdit>
	{
