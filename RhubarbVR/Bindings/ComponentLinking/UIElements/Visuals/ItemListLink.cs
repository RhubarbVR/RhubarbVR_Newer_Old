...
{
	public sealed class ItemListLink : UIElementLinkBase<RhuEngine.Components.ItemList, GDExtension.ItemList>
	{
		...
		private void IconMode_Changed(IChangeable obj) {
			RenderThread.ExecuteOnEndOfFrame(() => node.IconModeValue = LinkedComp.IconMode.Value switch { RItemListIconMode.Left => GDExtension.ItemList.IconMode.Left, _ => GDExtension.ItemList.IconMode.Top, });
		}
		...
		private void SelectMode_Changed(IChangeable obj) {
			RenderThread.ExecuteOnEndOfFrame(() => node.SelectModeValue = LinkedComp.SelectMode.Value switch { RItemListSelectMode.Multi => GDExtension.ItemList.SelectMode.Multi, _ => GDExtension.ItemList.SelectMode.Single, });
		}
	}
}
