...
namespace RhubarbVR.Bindings
{
	...
	public sealed class GodotMesh : IRMesh
	{
		...
		public static (Mesh.PrimitiveType, Array) CreateSubmesh(RPrimitiveType rPrimitiveType, Vector3f[][] uvs, int[] indexs, Vector3[] vectors, Vector3[] normals, Color[] color, Vector4[] tangents, int[] bones, float[] wights) {
			...
			arrays[(int)Mesh.ArrayType.Vertex] = (PackedVector3Array)vectors.AsSpan();
			if (normals is not null) {
				arrays[(int)Mesh.ArrayType.Normal] = (PackedVector3Array)normals.AsSpan();
			}
			if (tangents is not null) {
				arrays[(int)Mesh.ArrayType.Tangent] = (PackedFloat32Array)tangents.SelectMany(GetData).ToArray().AsSpan();
			}
			if (color is not null) {
				arrays[(int)Mesh.ArrayType.Color] = (PackedColorArray)color.AsSpan();
			}
			if (bones is not null) {
				arrays[(int)Mesh.ArrayType.Bones] = (PackedInt32Array)bones.AsSpan();
			}
			if (wights is not null) {
				arrays[(int)Mesh.ArrayType.Weights] = (PackedFloat32Array)wights.AsSpan();
			}
			arrays[(int)Mesh.ArrayType.Index] = (PackedInt32Array)indexs.AsSpan();
			for (var i = 0; i < uvs.Length; i++) {
				if (i <= 1) {
					...
					if (i == 0) {
						arrays[(int)Mesh.ArrayType.TexUv] = (PackedVector2Array)array.AsSpan();
					}
					else {
						arrays[(int)Mesh.ArrayType.TexUv2] = (PackedVector2Array)array.AsSpan();
					}
					...
					arrays[curentIndex] = (PackedFloat32Array)array.AsSpan();
				}
			}
				...
				if (complexMesh.HasMeshAttachments && complexMesh.MorphingMethod != RMeshMorphingMethod.None) {
					if (complexMesh.MorphingMethod == RMeshMorphingMethod.VertexBlend) {
						...
						foreach (var item in complexMesh.MeshAttachments) {
							...
							var smallist = System.Math.Min(item.Vertices.Count, complexMesh.VertexCount);
							for (var i = 0; i < smallist; i++) {
								verts[i] = new Vector3(item.Vertices[i].x, item.Vertices[i].y, item.Vertices[i].z) - cvertices[i];
							}
							shapedata[0] = (PackedVector3Array)verts.AsSpan();
							var norms = new Vector3[complexMesh.VertexCount];
							var smallistnorm = System.Math.Min(item.Normals.Count, complexMesh.VertexCount);
							...
							if (cnormals is not null) {
								shapedata[1] = (PackedVector3Array)norms.AsSpan();
							}

							var tangents = new Vector3[complexMesh.VertexCount];
							var smallitsTang = System.Math.Min(complexMesh.Tangents.Count, complexMesh.VertexCount);
							...
							if (ctangents is not null) {
								shapedata[2] = (PackedVector3Array)tangents.AsSpan();
							}
